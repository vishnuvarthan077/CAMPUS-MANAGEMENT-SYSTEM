...
const Dashboard = () => {
  ...
  const [counts, setCounts] = useState({ students: null, departments: null, faculty: null, courses: null });

  useEffect(() => {
    if (currentUser.role === 'student') return;
    Promise.all([
      studentService.getStudents({ limit: 1 }).catch(() => null),
      getDepartments().catch(() => null),
      facultyService.getFaculty({ limit: 1 }).catch(() => null),
      getCourses().catch(() => null),
    ]).then(([studentsRes, deptRes, facultyRes, coursesRes]) => {
      setCounts({
        students: studentsRes?.total ?? null,
        departments: deptRes?.count ?? null,
        faculty: facultyRes?.total ?? null,
  ...
  if (currentUser.role === 'student') {
    const studentStats = [
      { title: 'Department', value: currentUser.department || 'Not set', change: 'From your profile', icon: <Building2 size={24} />, path: '/departments', type: 'primary' },
      { title: 'Roll / Register Number', value: currentUser.registerNumber || 'Not set', change: 'From your profile', icon: <GraduationCap size={24} />, path: '/students', type: 'info' },
      { title: 'Browse Courses', value: 'Course Catalog', change: 'View by department & semester', icon: <BookOpen size={24} />, path: '/courses', type: 'success' },
      { title: 'Attendance & GPA', value: 'Demo data', change: 'Attendance/grading module coming soon', icon: <CheckSquare size={24} />, path: '/attendance', type: 'warning' },
    ];
    ...
    return (
      <div className="page">
        ...
        <div className="alert-error" style={{ background: 'var(--primary-light)', color: 'var(--primary)', borderColor: 'transparent' }}>
          <Info size={18} />
          <span>Today's timetable and course grades below are illustrative demo data — attendance and grading modules aren't wired to the backend yet.</span>
        </div>
  ...
  const stats = [
    {
      ...
      count: formatCount(counts.students),
      icon: <Users size={24} />,
      path: '/students',
      type: 'primary',
    },
    {
      ...
      count: formatCount(counts.departments),
    ...
    {
      title: 'Faculty Staff',
      count: formatCount(counts.faculty),
    ...
    },
    {
      ...
      count: formatCount(counts.courses),
        ...
        <div className="card">
          ...
          <div className="empty-state" style={{ padding: '1.5rem' }}>
            ...
            <p style={{ marginTop: '0.5rem', fontSize: '0.85rem', color: 'var(--text-muted)' }}>
              Activity logging isn't wired to the backend yet — this panel will show real events once it is.
            </p>
          </div>
        </div>
      </div>
